...
(async () => {
    ...
    async function getBlockList(scheduleNo) {
        ...
        const res = await axios_1.default.post("https://ticket.melon.com/tktapi/product/getAreaMap.json?v=1&callback=getBlockGradeSeatMapCallBack", body, { headers: baseHeaders });
    ...
    async function getRemainSeatInBlock(scheduleNo, block) {
        ...
        const res = await axios_1.default.post("https://ticket.melon.com/tktapi/product/seat/seatMapList.json?v=1&callback=getSeatListCallBack", body, { headers: baseHeaders });
        const json = String(res.data)
    ...
    for (const scheduleNo of scheduleNos) {
        ...
        for (const block of blocks) {
            ...
            if (count > 0) {
                await sendMessage(webhookUrl, `[${day}] ${block.sntv.a} 구역에 잔여 좌석 ${count}개 발생! https://ticket.melon.com/performance/index.htm?prodId=${prodId}`);
            }
        }
    }
    ...
    axios_1.default.post(webhookUrl, { content: `오류 발생! ${e.message}` });
